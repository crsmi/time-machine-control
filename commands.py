...
class SerialTo(serial.threaded.Protocol):
    """serial->GUI"""
...
class TimeMachine():
    ...
    """FUTURE IMPLEMENTATION
    def connect(self, com_port = None, listening_interface = None):
        # TODO: Make sure no current connection
        # Set COM Port
        if com_port:
            self.com_port = com_port
        ...
        if self.com_port:
            ...
            # Set up listening
            if listening_interface:
                self.ser_to = SerialTo()
                self.serial_worker = serial.threaded.ReaderThread(self.ser,
                                                                  self.ser_to)
                self.serial_worker.start()

        else:
            sys.stderr.write('No COM Port set for TimeMachine.')
            sys.exit(1)
